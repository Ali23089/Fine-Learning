      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="bg-white rounded-xl shadow-lg p-6 transform transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl"
            >
              {/* Icon */}
              <div className="flex items-center justify-center w-16 h-16 bg-[#1434cb] rounded-full mx-auto">
                {benefit.icon}
              </div>

              {/* Title */}
              <h3 className="mt-6 text-xl font-bold text-center text-[#1434cb]">
                {benefit.title}
              </h3>
